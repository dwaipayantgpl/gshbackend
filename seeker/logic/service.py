...
async def get_specific_seeker_full_details(target_id: str, current_reg: Registration):
    ...
    reg_info = (
        ...
        .where((Registration.id == valid_uuid) | (Registration.account == valid_uuid))
    ...
    if reg_info.capacity == "personal":
        profile = (
            ...
            .where(SeekerPersonal.registration == seeker_id)
            .first()
            .run()
        )
        if profile:
        ...
        profile = (
            ...
            .where(SeekerInstitutional.registration == seeker_id)
            .first()
            .run()
        )
        if profile:
    ...
    s_prefs = (
        ...
        .where(SeekerPreferenceNew.registration == seeker_id)
        .prefetch(
    ...
    main_pref = s_prefs[0]
    matched_services = [
        {"id": str(p.service.id), "name": p.service.name} for p in s_prefs if p.service
    ]
    ...
    return {
        ...
        "requirements": {
            ...
            "gender_pref": (
                main_pref.requirements.gender if main_pref.requirements else "Any"
            ),
            "age_range": f"{main_pref.requirements.min_age or 0} - {main_pref.requirements.max_age or 100}",
            "experience_required": (
                main_pref.requirements.experience if main_pref.requirements else None
            ),
        },
        "additional_notes": {
            "skills_needed": (
                main_pref.helper_details.skills if main_pref.helper_details else ""
            )
        },
    }
...
async def get_matches_for_seeker_logic(user_id: str):
    ...
    s_prefs = (
        await SeekerPreferenceNew.objects()
        .where(SeekerPreferenceNew.registration == user_id)
        .prefetch(
            SeekerPreferenceNew.service,
            SeekerPreferenceNew.location,
            SeekerPreferenceNew.work,
    ...
    for h in potential_helpers:
        ...
        if h.work.work_mode == s_main.work.work_mode:
            score += 1
        if h.work.accommodation == s_main.work.accommodation:
            score += 1
        checks += 2

        # 3. Salary Range Match (Can seeker afford this helper?)
        if h.requirements.min_salary <= s_main.requirements.max_salary:
            score += 1
        checks += 1

        # 4. Gender Preference (Does helper match seeker's requirement?)
        if (
            s_main.requirements.gender == "Any"
            or h.requirements.gender == s_main.requirements.gender
        ):
            score += 1
        checks += 1
        ...
        if match_pct >= 60:
            ...
            if h_reg_id in grouped_results:
                grouped_results[h_reg_id]["match_details"]["matched_services"].append(
                    h.service.name
                )
                continue
            ...
            # --- DATA COLLECTION ---
            account_record = (
                await Account.objects().get(Account.id == h.registration.account).run()
            )
            h_profile = (
                await HelperPersonal.objects()
                .where(HelperPersonal.registration == h.registration.id)
                .first()
                .run()
            )
            ...
            grouped_results[h_reg_id] = {
                ...
                "location": {"city": h.location.city, "area": h.location.area},
                "work_details": {
                ...
                "requirements": {
                    ...
                    "skills": (
                        h.helperpreference_details.skills
                        if h.helperpreference_details
                        else ""
                    ),
                },
            }
