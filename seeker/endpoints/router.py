...
async def add_seeker_preferences(
    ...
    existing = (
        ...
        .where(SeekerPreferenceNew.registration == user.id)
        .first()
        .run()
    )
    if existing:
...
async def update_seeker_preferences(
            ...
            PreferenceWork.update(
                {
                    ...
                    PreferenceWork.accommodation: work_upd.get(
    ...
    if "skills" in data:
        await (
            HelperDetails.update({HelperDetails.skills: data.get("skills")})
            .where(HelperDetails.id == m_det)
            .run()
        )
    ...
    final_prefs_count = (
        await SeekerPreferenceNew.count()
        .where(SeekerPreferenceNew.registration == user.id)
        .run()
    )
...
async def get_my_preferences(user: Registration = Depends(get_current_registration)):
    ...
    return {
        ...
        "details": {
            ...
            "requirements": (
                first.requirements.to_dict() if first.requirements else None
            ),
            "helper_details": (
                first.helper_details.to_dict() if first.helper_details else None
            ),
        },
    }
...
async def delete_seeker_preference(
    service_id: uuid.UUID, user: Registration = Depends(get_current_registration)
):
    ...
    await (
        SeekerPreferenceNew.delete()
        .where(SeekerPreferenceNew.id == pref_to_delete.id)
        .run()
    )
    ...
    if len(all_user_prefs) == 1:
        ...
        return {
            "message": "Deleted the last service and cleaned up all associated details."
        }

    return {
        "message": f"Service removed. {len(all_user_prefs) - 1} services remaining."
    }
...
async def get_helper_feed(user: Registration = Depends(get_current_registration)):
    ...
    seeker_prefs = (
        await SeekerPreference.select(
            SeekerPreference.service, SeekerPreference.city, SeekerPreference.area
        )
        .where(SeekerPreference.registration == user.id)
        .run()
    )

    if not seeker_prefs:
        return {
            "message": "No preferences set. Add some to see helpers!",
            "helpers": [],
        }
    ...
    for h in matching_helpers:
        ...
        ProfileTable = (
            HelperPersonal if h["capacity"] == "personal" else HelperInstitutional
        )
        profile = (
            await ProfileTable.objects()
            .where(ProfileTable.registration == h["helper_id"])
            .first()
            .run()
        )
...
async def find_my_helpers(user: Registration = Depends(get_current_registration)):
    ...
    if user.role != "seeker":
        raise HTTPException(
            status_code=403, detail="Only Seekers can search for Helpers."
        )
