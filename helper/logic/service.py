...
async def _validate_service_ids(service_ids: list[str]) -> None:
    ...
    rows = (
        await Service.objects().where(Service.id.is_in(service_ids)).columns(Service.id)
    )
...
def _validate_years(year_from: int | None, year_to: int | None) -> None:
    if year_from is not None and year_to is not None and year_from > year_to:
        raise HTTPException(
            status_code=400, detail="year_from cannot be greater than year_to"
        )
...
async def get_preference_by_registration_id(*, registration_id: str) -> dict:
    pref = (
        ...
        .where(HelperPreference.registration == registration_id)
        .first()
    )
    ...
    pref_services = (
        await HelperPreferredService.objects()
        .where(HelperPreferredService.registration == registration_id)
...
async def upsert_my_preference(*, account_id: str, payload: dict) -> dict:
    ...
    existing = (
        await HelperPreference.objects()
        .where(HelperPreference.registration == reg.id)
        .first()
    )
...
async def update_my_experience(
    *, account_id: str, experience_id: str, payload: dict
) -> dict:
    reg = await _get_registration_by_account_id(account_id)
    _ensure_helper(reg)

    row = (
        await HelperExperience.objects()
        .where(HelperExperience.id == experience_id)
        .first()
    )
...
async def add_helper_preference_logic(data: Dict[str, Any], user_id: str):
    ...
    existing = (
        await HelperPreference.objects()
        .where(HelperPreference.registration == user_id)
        .first()
        .run()
    )
    if existing:
        raise HTTPException(
            status_code=400, detail="Preferences already exist. Use PATCH to update."
        )
    ...
    loc_row = await PreferenceLocation.objects().create(
        city=loc_data.get("city"), area=loc_data.get("area")
    )
    ...
    if not service_ids:
        raise HTTPException(
            status_code=400, detail="At least one service_id is required."
        )
...
async def update_helper_preference_logic(data: Dict[str, Any], user_id: str):
    ...
    if any(k in data for k in ["work_schedule", "job_type", "work_mode"]):
        ...
        await (
            ...
            .where(PreferenceWork.id == m_work)
            .run()
        )
    ...
    if any(k in data for k in ["salary_range", "age_range", "gender", "experience"]):
        ...
        await (
            ...
            .where(PreferenceRequirements.id == m_req)
            .run()
        )
    ...
    if any(k in data for k in ["skills", "special_preferences"]):
        await (
            HelperSpecialPreferences.update(
                {
                    HelperSpecialPreferences.skills: str(data.get("skills")),
                    HelperSpecialPreferences.special_preferences: str(
                        data.get("special_preferences")
                    ),
                }
            )
            .where(HelperSpecialPreferences.id == m_det)
            .run()
        )
...
async def get_helper_preference_logic(user_id: str):
    ...
    if reg.capacity == "personal":
        profile = (
            ...
            .where(HelperPersonal.registration == user_id)
            .first()
            .run()
        )
        if profile:
        ...
        profile = (
            ...
            .where(HelperInstitutional.registration == user_id)
            .first()
            .run()
        )
    ...
    return {
        ...
        "details": {
            ...
            "helper_details": (
                first.helperpreference_details.to_dict()
                if first.helperpreference_details
                else None
            ),
        },
    }
...
async def get_matches_for_helper_logic(user_id: str):
    ...
    for s in potential_seekers:
        ...
        if s.work.work_mode == h_main.work.work_mode:
            score += 1
        if s.work.accommodation == h_main.work.accommodation:
            score += 1
        checks += 2

        if s.requirements.max_salary >= h_main.requirements.min_salary:
            score += 1
        checks += 1
        ...
        if match_pct >= 60:
            ...
            if s_reg_id in grouped_results:
                grouped_results[s_reg_id]["match_details"]["matched_services"].append(
                    s.service.name
                )
                # Keep the highest match score found
                current_score = int(
                    grouped_results[s_reg_id]["match_details"]["score"].replace("%", "")
                )
                if match_pct > current_score:
                    grouped_results[s_reg_id]["match_details"][
                        "score"
                    ] = f"{round(match_pct)}%"
                continue
            ...
            account_record = (
                await Account.objects().get(Account.id == s.registration.account).run()
            )
            ...
            if s.registration.capacity == "personal":
                s_profile = (
                    ...
                    .where(SeekerPersonal.registration == s.registration.id)
                    .first()
                    .run()
                )
            else:
                s_profile = (
                    ...
                    .where(SeekerInstitutional.registration == s.registration.id)
                    .first()
                    .run()
                )
            ...
            grouped_results[s_reg_id] = {
                ...
                "seeker_info": {
                    ...
                    "institution_type": (
                        getattr(s_profile, "institution_type", None)
                        if s.registration.capacity == "institutional"
                        else None
                    ),
                    "contact_phone": contact_phone,
                },
                "location": {"city": s.location.city, "area": s.location.area},
                "work_details": {
                ...
                "requirements": {
                    ...
                    "skills_required": (
                        s.helper_details.skills if s.helper_details else ""
                    ),
                },
            }
    ...
    return sorted(
        final_results,
        key=lambda x: int(x["match_details"]["score"].replace("%", "")),
        reverse=True,
    )
...
async def get_specific_helper_full_details(target_id: str, current_reg: Registration):
    ...
    reg_info = (
        await Registration.objects()
        .where((Registration.id == target_id) | (Registration.account == target_id))
        .prefetch(Registration.account)
        .first()
        .run()
    )
    ...
    if reg_info.capacity == "personal":
        profile = (
            await HelperPersonal.objects()
            .where(HelperPersonal.registration == helper_id)
            .first()
            .run()
        )
        if profile:
        ...
        profile = (
            await HelperInstitutional.objects()
            .where(HelperInstitutional.registration == helper_id)
            .first()
            .run()
        )
        if profile:
    ...
    h_prefs = (
        await HelperPreference.objects()
        .where(HelperPreference.registration == helper_id)
        .prefetch(
            HelperPreference.service,
            HelperPreference.location,
            HelperPreference.work,
    ...
    return {
        ...
        "requirements": {
            "min_salary": (
                main_pref.requirements.min_salary if main_pref.requirements else None
            ),
            "max_salary": (
                main_pref.requirements.max_salary if main_pref.requirements else None
            ),
            "gender_pref": (
                main_pref.requirements.gender if main_pref.requirements else None
            ),  # This is where gender lives
        },
        "additional_details": {
            "skills": (
                main_pref.helperpreference_details.skills
                if main_pref.helperpreference_details
                else None
            ),
            "special_preferences": (
                main_pref.helperpreference_details.special_preferences
                if main_pref.helperpreference_details
                else None
            ),
        },
