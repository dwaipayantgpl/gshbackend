...
from complaint.logic.service import get_all_complaints, update_complaint_status, save_complaint_proof_logic
...
@router.get("/admin/usercomplaintsdata", response_model=List[ComplaintOut], summary="Admin: View all complaints")
...
@router.get("/history/{target_registration_id}", response_model=List[ComplaintOut], summary="Admin or Owner: View history")
async def get_complaint_history(
    ...
    target_reg = await Registration.objects().where(
        Registration.id == UUID(target_registration_id)
    ).first().run()
    ...
    is_admin = False
    current_reg = await Registration.objects().where(
        Registration.account == UUID(current_account_id)
    ).first().run()
