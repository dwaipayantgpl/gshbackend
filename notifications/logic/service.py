...
class NotificationService:
    ...
    async def get_unread_count(user_id: str):
        ...
        count = await Notifiactions.count().where(
            (Notifiactions.recipient == user_id) & 
            (Notifiactions.is_read == False)
        ).run()
        return count
    ...
    @staticmethod
    async def broadcast_new_service(service_name: str, description: str, service_id: str = None):
