...
from bookings.structs.dtos import BookingUpdateSchema, DirectBookingCreate,BookingSummaryOut
...
@router.patch("/{booking_id}/update", summary="Seeker updates booking details (Only if pending)")
...
async def get_my_bookings(
    ...
    """
    if current_user.role not in ['seeker', 'both']:
        raise HTTPException(status_code=403, detail="Only seekers can view booking history")
...
@router.get("/helper/{helper_id}/busy-dates", summary="Get unavailable dates for a helper")
...
async def get_helper_requests(current_user: Registration = Depends(get_current_registration)):
    # Fetch all bookings assigned to this helper that are still 'pending'
    requests = await ServiceBooking.select(
...
async def helper_respond_to_booking(
    ...
    if current_user.role not in ['helper', 'both']:
        raise HTTPException(status_code=403, detail="Only helpers can respond to requests.")
...
async def get_helper_booking_requests(
    ...
    """
    if current_user.role not in ['helper', 'both']:
        raise HTTPException(status_code=403, detail="Only helpers can view these requests")
